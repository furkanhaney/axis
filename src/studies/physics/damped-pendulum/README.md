...
Adam updates over 256,000 fresh, exact `f32` center-coordinate identities from
an explicitly finite shuffled lattice, then applies the declared residual gates
on a fixed 255-point interior grid. The source declares its available population
and a 10% maximum coverage; the measured run uses about 6.1%. It rejects every
train/evaluation stencil overlap and ends rather than wrapping at exhaustion.
...
to `0.00749136` radians and angular-velocity RMSE fell from `1.60544326` to
`0.01102462` radians/second against an independent f64 RK4 solver. The executable
...
`0.07986000` radians/second², maximum absolute residual `0.19565988`, and no
observation exceeded the strict `0.25` radians/second² tolerance. The receipt
